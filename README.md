...
- 本机控制台：关键词、邮箱、API、SMTP、来源和计划均可视化维护；「发送记录」面板可直接打开历次日报/仪表盘，并提供回收目录清理入口。
- LLM 结果缓存：DeepSeek 逐篇解读结果保存在本机 `llm_cache.json`，多个方案与失败重跑不再重复消耗配额；LLM 不可用（未配置 Key/余额不足/限流）时报告顶部会显示原因并回退本地规则解读。
